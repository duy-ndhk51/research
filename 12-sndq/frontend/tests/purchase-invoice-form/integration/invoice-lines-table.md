...
- **Rounding: non-round amount at 6% VAT** / **Rounding: multi-line mixed rates with odd cents** / **Round-trip: stored `amount` may not reproduce `totalAmount` via forward calc** guard **rounding edge cases** -- `calculateSubtotalFromTotal` (reverse) and `calculatePurchaseInvoiceAmount` (forward) use different rounding paths; for certain amount/vatRate combinations (e.g. 3333 cents at 6%) the round-trip can drift by ±1 cent; footer uses the difference method (`totalAmount - exclVat`) which masks the drift in the UI but the stored `amount` field may be inconsistent with forward calculation
...
| Rounding: non-round amount at 6% VAT | Footer subtotal + VAT row = total (no 1-cent drift visible) |
| Rounding: multi-line mixed rates with odd cents | Footer grand total = sum of `totalAmount` fields, per-rate VAT rows present |
| Round-trip: stored `amount` may not reproduce `totalAmount` via forward calc | `calculatePurchaseInvoiceAmount(line).totalAmount` may differ from `line.totalAmount` by ±1 |
...
**Preconditions**: 1 line with `totalAmount: 3333`, `vatRate: 6`, `hasVat: true`.

### Steps

1. Render with the line described above
2. Inspect the footer breakdown values

### Expected Outcome (current behavior)

- `calculateSubtotalFromTotal(3333, 6)` returns `3145` -- stored as `line.amount`
- Footer displays subtotal: `€31.45` (3145 cents)
- Footer displays VAT 6%: `€1.88` (188 cents, via difference: `3333 - 3145`)
- Footer displays total: `€33.33` (3333 cents)
- Subtotal + VAT in footer = total (188 + 3145 = 3333) -- **looks correct in UI**
...
**Preconditions**: 3 lines with mixed VAT rates and odd cent totals.

### Steps

1. Render with 3 lines:
   - Line A: `totalAmount: 3333, amount: 3333, hasVat: false, vatRate: 0`
   - Line B: `totalAmount: 5555, amount: 5240, hasVat: true, vatRate: 6`
   - Line C: `totalAmount: 7777, amount: 6428, hasVat: true, vatRate: 21`
2. Inspect footer breakdown

### Expected Outcome (current behavior)

- Footer grand total: `€166.65` (16665 cents = sum of all `totalAmount` fields)
- `useLineTotals` computes `totalExclVat` via `calculatePurchaseInvoiceAmount` per line:
  - Line A: `exclVat = 3333` (no VAT)
  - Line B: `calculateSubtotalFromTotal(5555, 6) = 5240` -> `exclVat = 5240`
  - Line C: `calculateSubtotalFromTotal(7777, 21) = 6428` -> `exclVat = 6428` (verify: `Math.round(7777 * 10000 / 12100) = 6427.27... = 6427`... need to verify exact value)
- Footer shows VAT rows for 6% and 21%
- `totalInclVat` = sum of `totalAmount` = 16665 (this is always correct since it sums raw values)
...
it('multi-line mixed rates with odd cent totals', () => {
  ...
  // Line B: Math.round((5555 * 10000) / 10600) = Math.round(52405.66...) = 52406 / 10 -> 5241
  // Actually: Math.round(55550000 / 10600) = Math.round(5240.566...) = 5241
  ...
  // Line B: 5241 + Math.round(5241 * 6 / 100) = 5241 + 314 = 5555 (matches!)
  // Line C: 6427 + Math.round(6427 * 21 / 100) = 6427 + 1350 = 7777 (matches!)
  // These specific values happen to round-trip cleanly, but other combos may not.
